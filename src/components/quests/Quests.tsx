...
export const Quests = () => {
  const { fetchQuestData } = questStore();
  const [showModal, setShowModal] = useState(false);

  // Set featured quest and get its index
  const featuredPartner = Partner.Solv;
  const featuredIndex = QuestsInfo.findIndex(
    (quest) => quest.partner === featuredPartner
  );
  ...
  return (
    <div className="mx-auto mt-10 w-full max-w-[1600px] px-10">
      <FeaturedQuest
